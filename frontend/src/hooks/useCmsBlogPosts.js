...
const extractPosts = (response) => {
  ...
  if (response.data?.blogPosts) {
    ...
    if (Array.isArray(response.data.blogPosts.edges))
      return extractEdges(response.data.blogPosts.edges);
  }
...
const extractPagination = (response) => {
  ...
  if (response.meta?.pagination && typeof response.meta.pagination === "object")
    return response.meta.pagination;
  if (response.data?.pagination && typeof response.data.pagination === "object")
    return response.data.pagination;
  if (
    response.data?.posts?.meta?.pagination &&
    typeof response.data.posts.meta.pagination === "object"
  ) {
  ...
  if (
    response.data?.blogPosts?.meta?.pagination &&
    typeof response.data.blogPosts.meta.pagination === "object"
  ) {
    return response.data.blogPosts.meta.pagination;
  }
  if (
    response.data?.blogPosts?.pagination &&
    typeof response.data.blogPosts.pagination === "object"
  ) {
...
const resolvePublishedAt = (post) => {
  if (!post || typeof post !== "object") return null;
  return (
    post.publishedAt ?? post.published_at ?? post.date ?? post.created_at ?? post.createdAt ?? null
  );
};
...
const normalisePostSummary = (post) => {
  ...
  return {
    ...
    slug: ensureValue(post.slug, ensureValue(post.permalink, ensureValue(post.path, undefined))),
    title: ensureValue(
      post.title,
      ensureValue(post.name, ensureValue(post.heading, "Ongetiteld artikel")),
    ),
    excerpt: ensureValue(
      post.excerpt,
      ensureValue(post.summary, ensureValue(post.description, post.teaser ?? "")),
    ),
    ...
    async (targetPage, signal) => {
      ...
      const resolvedPagination = paginationInfo
        ...
        : createPagination(
            targetPage,
            pageSize,
            ensureValue(response?.total, normalisedPosts.length),
          );
      ...
      .catch((fetchError) => {
        ...
        const normalisedError =
          fetchError instanceof Error
            ? fetchError
            : new Error("Onbekende fout bij het laden van blogartikelen");
