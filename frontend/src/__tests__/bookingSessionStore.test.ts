...
import { bookingSessionStore, bookingSessionTestHelpers } from "../lib/bookingSessionStore";
...
describe("bookingSessionStore", () => {
  ...
  it("restores the previous state when an update fails", async () => {
    ...
    await expect(bookingSessionStore.updateSelections({ location: "Breda" })).rejects.toThrow(
      "Timeout",
    );
