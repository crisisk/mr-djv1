...
const BlogListItem = ({ post }) => {
  ...
  const displayReadingTime = resolveReadingTime(readingTime ?? post?.meta?.readingTime ?? post?.read_time)
  const formattedDate = formatDate(publishedAt ?? post?.date ?? post?.created_at ?? post?.createdAt)
  const metadataParts = []
  ...
  return (
    <article className={styles.item}>
      <a className={styles.thumbnailLink} href={href} aria-label={`Lees ${title ?? 'dit artikel'}`}>
        {thumbnailUrl ? (
          <img className={styles.thumbnailImage} src={thumbnailUrl} alt={title ? `Thumbnail voor ${title}` : 'Blog thumbnail'} />
        ) : (
