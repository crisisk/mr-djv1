...
const { createCacheMiddleware } = require('../middleware/redisCache');

const router = express.Router();

// Cache metrics for 30 seconds (balance between freshness and performance)
const metricsCache = createCacheMiddleware({ ttl: 30, prefix: 'metrics:' });

router.get('/queues', metricsCache, async (_req, res, next) => {
  try {
