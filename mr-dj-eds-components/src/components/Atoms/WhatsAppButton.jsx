...
const WhatsAppButton = () => {
  ...
  const trackConversion = async () => {
    try {
      const module = await loadTrackConversion();
      if (typeof module?.trackWhatsAppClick === 'function') {
        module.trackWhatsAppClick('A');
      }
    } catch (error) {
      logger.error('Failed to load tracking utilities for WhatsApp click', error);
    }
  };
  ...
  return (
    <a
      ...
      target={target}
      rel={rel}
      aria-label={config.isValid ? 'Chat via WhatsApp' : 'Open het contactformulier'}
      onClick={(event) => {
    ...
    >
      ...
      <span className="absolute right-full mr-3 top-1/2 -translate-y-1/2 bg-neutral-dark text-white px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap opacity-0 group-hover:opacity-100 group-focus-visible:opacity-100 transition-opacity duration-300 pointer-events-none shadow-lg">
        {config.isValid ? 'Chat via WhatsApp' : 'Contactformulier'}
      </span>
